                    ...
                    <div class="col-lg-12">
                        <div class="card custom-card overflow-hidden">
                            <div class="card-body" style="background-color: #0000000a;border-radius: 10px 10px 0px 0px;">
                                <div class="row">
                                    <div class="col"><a href="{{url()->current()}}" class="btn btn-link btn-xs">لیست مقالات </a></div>
                                    <div class="col text-left"><a href="{{url(request()->segment(1).'/'.request()->segment(2).'/'.'create')}}" class="btn btn-primary btn-xs">+ افزودن مقالات </a></div>
                                </div>
                            </div>
                            <div class="card-body">
...
@foreach($blugs as $blug)
    <div id="myModal{{$blug->id}}" class="modal fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="deleteblug{{$blug->id}}" class="btn btn-danger" data-id="{{$blug->id}} " data-dismiss="modal">بله</button>
                </div>
            </div>
    ...
    @foreach($blugs as $blug)
        <script>
            jQuery(document).ready(function(){
                jQuery('#deleteblug{{$blug->id}}').click(function(e){
                    e.preventDefault();
                    $.ajaxSetup({
                        headers: {
