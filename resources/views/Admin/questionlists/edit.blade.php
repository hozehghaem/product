                ...
                <div class="row row-sm">
                    <div class="col-lg-12 col-md-12">
                        <div class="card custom-card">
                            ...
                            <div class="card-body">
                                <form action="{{route(request()->segment(2).'.'.'update', $questionlists->id)}}" method="POST">
                                    {{csrf_field()}}
                                    {{ method_field('PATCH') }}
                                    <div class="row row-sm">
                                        <div class="col-md-12">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group" >
                                                ...
                                                <textarea name="question" id="question" cols="30" rows="5" class="form-control" >{{$questionlists->question}}</textarea>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group" >
                                                <p class="mg-b-10">متن پاسخ</p>
                                                <textarea name="answer" id="answer" cols="30" rows="5" class="form-control" >{{$questionlists->answer}}</textarea>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select name="status" id="status" class="form-control select-lg select2">
                                                    <option value="4" {{$questionlists->status = 4 ? 'selected' : ''}}>فعال</option>
                                                    <option value="0" {{$questionlists->status = 0 ? 'selected' : ''}}>غیر فعال</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select name="submenu_id" id="submenu_id" class="form-control select-lg select2">
                                                    ...
                                                    @foreach($submenus as $submenu)
                                                        <option value="{{$submenu->id}}" {{$questionlists->submenu_id == $submenu->id ? 'selected' : ''}}>{{$submenu->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
