                ...
                <div class="row row-sm">
                    <div class="col-lg-12 col-md-12">
                        <div class="card custom-card">
                            ...
                            <div class="card-body">
                                <form action="{{route(request()->segment(2).'.update', $megamenus->id)}}" method="POST">
                                    <div class="row row-sm">
                                        {{csrf_field()}}
                                        {{ method_field('PATCH') }}
                                        <div class="col-md-12">
{{--                                               @include('error')--}}
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="title" id="title" value="{{$megamenus->title}}" class="form-control" />
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select name="menu_id" id="menu_id" class="form-control select-lg select2">
                                                    ...
                                                    @foreach($menus as $menu)
                                                        <option value="{{$menu->id}}" {{$menu->id == $megamenus->menu_id ? 'selected' : ''}}>{{$menu->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
    ...
    <script>
        jQuery(document).ready(function(){
            jQuery('#submit').click(function(e){
                ...
                jQuery.ajax({
                    url: "{{ route(request()->segment(2).'.update' , $megamenus->id) }}",
                    method: 'PATCH',
                    data: {
                        "_token": "{{ csrf_token() }}",
